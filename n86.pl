...
#
# This is N86, the subroutine that converts Newkirk 1986 notation into
...
sub getn {
    ...
    unless(open (DATA, "/home/grvsmth/public_html/signsynth/dev/n86.txt")) {
	  ...
	  if (substr ($keyas, 0, 1) eq "\$"){ 
        ...
        close (DATA);
#	print "Leaving getn(); \$rthumb1k{C} is $rthumb1k{C}<P>";
...
#      print "\$relbow {$time4} is $relbow{$time4}\n";
    }
...
#now analyze each sign
  ...
  foreach $num (0 .. $length) {
    if ($debugem) {print "Working on sign $num: $sign[$num]<P>\n";}
...
# if it was, we do the facials for the last one.
    ...
    if ($debugem) {print "\$lastwd is $lastwd<P>";}
...
#   Now I parse the facial stuff; the head and body come later
    ...
    @fparam = ("Xb","Xi","Xey","Xex","Xm","Ys","Yk","Yty","Ytx");

    @fvalues = ("bdg","yrl","iu","oae","njxmwts","ptkgcbd","fsh","iu","oae");
    ...
    foreach $i (0 .. $flen-1) {
      if ($debugem) {print "\$fcode[$i] is $fcode[$i]<P>";}
      foreach $j ($jplace .. 8) {
	if ($debugem) {
